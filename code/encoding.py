...
def predict(rate_pred2, test_y): # compute reduced mse compared with homogeneous poisson
    return 1-(((rate_pred2 - test_y)**2).mean(axis=0)/np.var(test_y,axis=0)).mean();

def get_tc(y, hd, bin_len=181): # compute empirical tunning curve of data
...
def poissonEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate):
    DTYPE = tf.float32
    #yDim = tf.shape(Y)[1];
    #xDim = tf.shape(X)[1];
    ...
    with tf.variable_scope("poi_rate_nn", reuse=tf.AUTO_REUSE):
        full1 = fullyConnectedLayer(X, gen_nodes, nl='tanh', scope='full1',
                initializer=tf.random_uniform_initializer(minval=-rangeRate1,maxval=rangeRate1));
        full2 = fullyConnectedLayer(full1, gen_nodes, nl='tanh', scope='full2',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
        full = fullyConnectedLayer(full2, yDim, nl='linear', scope='output',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
...
def sngEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate, factor, gamma_np):
    DTYPE = tf.float32
    #yDim = tf.shape(Y)[1];
    #xDim = tf.shape(X)[1];
...
def sngRlxEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate, factor):
    DTYPE = tf.float32
    #yDim = tf.shape(Y)[1];
    #xDim = tf.shape(X)[1];
    ...
    with tf.variable_scope("sngrlx_rate_nn", reuse=tf.AUTO_REUSE):
        ...
        full_theta = fullyConnectedLayer(full2, yDim, nl='linear', scope='output_theta',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
        full_p = fullyConnectedLayer(full2, yDim, nl='linear', scope='output_p',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
...
def gammaEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate):
    ...
    with tf.variable_scope("gamma_rate_nn", reuse=tf.AUTO_REUSE):
        ...
        full_theta = fullyConnectedLayer(full2, yDim, nl='linear', scope='output_theta',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
        full_k = fullyConnectedLayer(full2, yDim, nl='linear', scope='output_k',
                initializer=tf.random_uniform_initializer(minval=-rangeRate2,maxval=rangeRate2));
...
def sampleGamma(k, theta, seed=888):
    np.random.seed(seed);
    y_gamma = np.random.gamma(scale=theta,shape=k);
    return y_gamma
...
def poissonRunner(Y_train, Y_valid, hd_train, hd_valid, hd_test, n_epochs=800, learning_rate=1e-2, gen_nodes=60, bin_len=181, path='./rlt/'):
    #nTrain = 14600;
    ...
    hd_angle_train = np.vstack((np.cos(hd_train*np.pi/180), np.sin(hd_train*np.pi/180))).T;
    hd_angle_valid = np.vstack((np.cos(hd_valid*np.pi/180), np.sin(hd_valid*np.pi/180))).T;
    hd_angle_test = np.vstack((np.cos(hd_test*np.pi/180), np.sin(hd_test*np.pi/180))).T;
    ...
    optimizer, entropy_loss, rate = poissonEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate);
    ...
    hd_bins_angle =  np.vstack((np.cos(hd_bins_use*np.pi/180), np.sin(hd_bins_use*np.pi/180))).T;
...
def bernoulliRunner(Y_train_use, Y_valid_use, hd_train, hd_valid, hd_test, thresh, n_epochs=800, learning_rate=1e-2, gen_nodes=60, bin_len=181, path='./rlt/'):
    #nTrain = 14600;
    name = path+'ber.ckpt';
    #if (isinstance(thresh, float) | isinstance(thresh, int)):
    #Y_train = (Y_train_use>thresh);
    #Y_valid = (Y_valid_use>thresh);
    Y_train = (Y_train_use!=0);
    Y_valid = (Y_valid_use!=0);
    nTrain = Y_train.shape[0];
    nValid = Y_valid.shape[0];
    ...
    optimizer, entropy_loss, rate = bernoulliEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate);
...
def sngRunner(Y_train, Y_valid, hd_train, hd_valid, hd_test, factor, n_epochs=800, learning_rate=1e-2, gen_nodes=60, bin_len=181, path='./rlt/'):
    #nTrain = 14600;
    name = path+'sng.ckpt';
    nTrain = Y_train.shape[0];
    nValid = Y_valid.shape[0];
    ...
    gamma[np.isnan(gamma)]=1e-7;
    
    Y = tf.placeholder(DTYPE, [None, yDim], name='Y');
    X = tf.placeholder(DTYPE, [None, xDim], name='X');

    optimizer, entropy_loss, rate, k, gamma, loc = sngEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate, factor, gamma);
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver();
...
def sngRlxRunner(Y_train, Y_valid, hd_train, hd_valid, hd_test, factor, n_epochs=800, learning_rate=1e-2, gen_nodes=60, bin_len=181, path='./rlt/'):
    #nTrain = 14600;
    name = path+'sng_rlx.ckpt';
    nTrain = Y_train.shape[0];
    nValid = Y_valid.shape[0];
    ...
    optimizer, entropy_loss, theta, k, p, loc, rate = sngRlxEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate,factor);
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver();
...
def gammaRunner(Y_train_use, Y_valid_use, hd_train, hd_valid, hd_test, factor=1e-7, n_epochs=800, learning_rate=1e-2, gen_nodes=60, bin_len=181, path='./rlt/'):
    #nTrain = 14600;
    name = path+'gamma.ckpt';
    Y_train = (Y_train_use + factor).copy();
    Y_valid = (Y_valid_use + factor).copy();
    nTrain = Y_train.shape[0];
    nValid = Y_valid.shape[0];
    ...
    optimizer, entropy_loss, theta, k, rate = gammaEncoding(Y, X, yDim, xDim, gen_nodes, learning_rate);
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver();
    ...
    return spl_values, spl_k_values, spl_theta_values, yfit, kfit, thetafit, cost, test_cost
...
def fitmle(rlt):
    ...
    print("p fit ", np.round(p_fit,2))
    print("k fit ", np.round(k_fit,2))
    print("loc fit ", np.round(loc_fit,2))
    print("a fit ", np.round(theta_fit,2))
    # plot
...
def get_var(y, hd, bins):
    ...
    for ii in range(len(hd_bins)-1):
        ...
        #rate_real_short.append(Y_real[:T][data_pos,:].mean(axis=0));
    var_est_short = np.asarray(var_est_short);
    #rate_real_short = np.asarray(rate_real_short).ravel();
    return var_est_short

def get_zero(y, hd, bins):
    hd_bins = np.linspace(-180,180,bins+1);
    T = y.shape[0];
    zero_est_short = [];
    data_use = np.zeros((len(hd_bins)-1,2));
    for ii in range(len(hd_bins)-1):
        ...
        #rate_real_short.append(Y_real[:T][data_pos,:].mean(axis=0));
    zero_est_short = np.asarray(zero_est_short);
    #rate_real_short = np.asarray(rate_real_short).ravel();
